...
assert(  operatingSystem    .platform()    .toLowerCase()    .includes('linux'),  `• This script must be run in WSL (wsl.exe), not Windows OS.`)
...
export const nonElevatedCallback = async () => {
  ...
  */
  ...
  } catch(error) { /* ignore - usually throws if no username is set or config file exist */ }
  try { 
